  ...
  return (
    ...
    >
      <div
        ...
        className="fixed top-0 z-30 h-screen w-96 overflow-y-auto rounded-r-2xl bg-white shadow-right dark:border-r dark:border-zinc-600 dark:bg-black-dark"
      >
        <div className="flex-column h-32 w-full border-b border-gray-base px-5">
          <div className="my-2 w-full">
            <h2 className="mb-8 mt-6 text-2xl font-medium dark:text-white">Search</h2>
            <input
              value={query}
              onChange={e => setQuery(e.target.value)}
              className="w-full rounded-lg border-gray-base bg-gray-100 px-3 py-2 outline-none placeholder:font-thin dark:bg-black-light dark:text-white"
              type="text"
              placeholder="Search"
            />
          </div>
        ...
        <div className="my-2 w-full px-6">
          <h4 className="font-medium dark:text-white">Resent</h4>
          {users === null ? (
            <p></p>
          ) : isLoading && users.length === 0 ? (
            <div className="mt-32 flex h-full justify-center">
              <CircleLoader size="2xl" color="gray-400" />
            </div>
          ) : users.length === 0 ? (
